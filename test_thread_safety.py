...
class TestThreadSafety(unittest.TestCase):
    ...
    def test_global_manager_access_thread_safety(self):
        ...
        for i in range(num_threads):
            thread = threading.Thread(target=access_managers, args=(i, iterations_per_thread))
            threads.append(thread)
            thread.start()
    ...
    def test_concurrent_file_operations(self):
        ...
        for i in range(num_threads):
            thread = threading.Thread(target=file_worker, args=(i, iterations_per_thread))
            threads.append(thread)
            thread.start()
        ...
        self.assertEqual(actual_files, expected_files, "All expected files should be created")
    ...
    def test_lru_cache_thread_safety(self):
        ...
        def cache_worker(worker_id: int, iterations: int):
            ...
            for i in range(iterations):
                ...
                self.assertEqual(result, i * 2, f"Cache function should return correct result for {i}")
        ...
        for i in range(num_threads):
            thread = threading.Thread(target=cache_worker, args=(i, iterations_per_thread))
            threads.append(thread)
            thread.start()
    ...
    def test_statistics_aggregation_consistency(self):
        ...
        for i in range(num_threads):
            thread = threading.Thread(target=stats_worker, args=(i, iterations_per_thread))
            threads.append(thread)
            thread.start()
        ...
        for key, value in final_stats.items():
            if key != "own_number":
                self.assertEqual(value, expected_total, f"Stats {key} should equal {expected_total}")
    ...
    def test_error_handling_in_threads(self):
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            ...
            self.assertGreater(completed, 0, "Some workers should complete successfully")
            self.assertGreater(errors, 0, "Some workers should encounter errors")
            self.assertEqual(completed + errors, 10, "Total should equal number of workers")
